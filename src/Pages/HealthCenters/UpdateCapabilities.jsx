...
const dummyCapabilities = {
  isOpen: true,
  postLossCare: { enabled: true,  reason: "" },
  bloodBank:    { enabled: true,  reason: "" },
  surgical:     { enabled: true,  reason: "" },
  maternity:    { enabled: true,  reason: "" },
  icu:          { enabled: false, reason: "ICU under renovation until June 2026" },
};

const dummyCapacity = { availableBeds: 12, staffOnDuty: 8, estimatedWaitMinutes: 20 };
...
export default function UpdateCapabilities() {
  ...
  const [isOpen, setIsOpen]             = useState(dummyCapabilities.isOpen);
  const [capabilities, setCapabilities] = useState({
    postLossCare: { ...dummyCapabilities.postLossCare },
    bloodBank:    { ...dummyCapabilities.bloodBank    },
    surgical:     { ...dummyCapabilities.surgical     },
    maternity:    { ...dummyCapabilities.maternity    },
    icu:          { ...dummyCapabilities.icu          },
  });
  const [capacity, setCapacity] = useState({ ...dummyCapacity });
  ...
  function toggleCap(key) {
    setCapabilities(p => ({ ...p, [key]: { ...p[key], enabled: !p[key].enabled } }));
  }
    ...
    <>
      ...
      <div className="uc-root">
        <div style={{ padding: "clamp(20px,4vw,40px) clamp(16px,4vw,40px) 0", maxWidth: 680, display: "flex", flexDirection: "column", gap: 28 }}>
          ...
          {/* Save */}
          <button onClick={() => showToast("Capabilities updated. The AI will use this information immediately.")}
